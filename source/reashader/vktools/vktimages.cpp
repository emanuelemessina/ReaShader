	...
	namespace Images
	{
		...
		AllocatedImage::AllocatedImage(Logical::Device* vktDevice, bool pushToDeviceDeletionQueue)
		   ...
		   void AllocatedImage::createImage(VkExtent2D extent, VkImageType type, VkFormat format,
							...
							VkMemoryPropertyFlags memoryProperties, VmaAllocationCreateFlags vmaFlags)
		   {
			   ...
			   uint32_t uniqueQueueFamilies[1] = {
				   vktDevice->physicalDevice->queueFamilyIndices.graphicsFamily.value()
			   };
							...
							VkPipelineStageFlags finalStageMask)
		   {
			   ...
			   Buffers::AllocatedBuffer* srcBuff = vkt::commands::createPixelBufferSrc(vktDevice, (char*)pixels, extent);

			   vkt::commands::transferPixelBufferToImage(cmd, srcBuff, image, extent, finalAccessMask, finalImageLayout,
														 finalStageMask);

			   // execute

			   VkFence fence = vkt::sync::createFence(vktDevice, false, false);
			   ...
			   srcBuff->destroy();
			   vkt::sync::destroySyncObject(vktDevice, fence);
