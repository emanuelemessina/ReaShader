...
bool LICE_MemBitmap::__resize(int w, int h)
{
	if (w != m_width || h != m_height)
	{
		...
		free(m_fb);
		m_fb = (LICE_pixel*)malloc((m_allocsize = ((w + m_linealign) & ~m_linealign) * h * sizeof(LICE_pixel)) +
...
{
  public:
	static void solidBlitFAST(LICE_pixel* dest, int w, int h, LICE_pixel color, int dest_span)
	{
		while (h--)
		{
	...
	static void scaleBlitFAST(LICE_pixel_chan* dest, const LICE_pixel_chan* src, int w, int h, int icurx, int icury,
							  int idx, int idy, unsigned int clipright, unsigned int clipbottom, int src_span,
							  int dest_span)
	{
...
#define DOPIX(pout, r, g, b, a, ia) COMBFUNC::doPix(pout, r, g, b, a, ia);
  ...
  public:
	...
	static void gradBlit(LICE_pixel_chan* dest, int w, int h, int ir, int ig, int ib, int ia, int drdx, int dgdx,
						 int dbdx, int dadx, int drdy, int dgdy, int dbdy, int dady, int dest_span
#ifdef LICE_FAVOR_SIZE_EXTREME
						 ,
						 LICE_COMBINEFUNC combFunc
#endif
...
#define DOPIX(pout, r, g, b, a, ia) COMBFUNC::doPix(pout, r, g, b, a, ia);
  ...
  public:
	static void blit(LICE_pixel_chan* dest, const LICE_pixel_chan* src, int w, int h, int src_span, int dest_span,
					 int ia
#ifdef LICE_FAVOR_SIZE
					 ,
					 LICE_COMBINEFUNC combFunc
	...
	// this is only designed for filtering down an image approx 2:1 to 4:1 or so.. it'll work (poortly) for higher, and
	// for less it's crap too. probably need to redo it using linear interpolation of the filter coefficients, but bleh
	// I'm gonna go play some call of duty
	static void scaleBlitFilterDown(LICE_pixel_chan* dest, const LICE_pixel_chan* src, int w, int h, int icurx,
									int icury, int idx, int idy, int clipright, int clipbottom, int src_span,
									int dest_span, int ia, const int* filter, int filt_start, int filtsz
#ifdef LICE_FAVOR_SIZE
									,
									LICE_COMBINEFUNC combFunc
#endif
			...
			if (cury >= 0 && cury < clipbottom)
			{
				...
				while (n--)
				{
					...
					if (offs >= 0 && offs < clipright)
					{
						...
						const LICE_pixel_chan* rdptr =
							inptr + (offs + filt_start) * (int)(sizeof(LICE_pixel) / sizeof(LICE_pixel_chan));
	...
	static void scaleBlit(LICE_pixel_chan* dest, const LICE_pixel_chan* src, int w, int h, int icurx, int icury,
						  int idx, int idy, unsigned int clipright, unsigned int clipbottom, int src_span,
						  int dest_span, int ia, int filtermode
#ifdef LICE_FAVOR_SIZE
						  ,
						  LICE_COMBINEFUNC combFunc
#endif
			...
			while (h--)
			{
				...
				else if (cury == clipbottom - 1)
				{
					while (n--)
					{
						...
						{
							int r, g, b, a;
							__LICE_LinearFilterI(&r, &g, &b, &a, pin,
												 pin + sizeof(LICE_pixel) / sizeof(LICE_pixel_chan), curx & 0xffff);
							DOPIX(pout, r, g, b, a, ia)
						}
						else if (offs == clipright - 1)
						{
...
#define DOPIX(pout, r, g, b, a, ia) COMBFUNC::doPix(pout, r, g, b, a, ia);
  ...
  public:
	static void deltaBlit(LICE_pixel_chan* dest, const LICE_pixel_chan* src, int w, int h, int isrcx, int isrcy,
						  int idsdx, int idtdx, int idsdy, int idtdy, int idsdxdy, int idtdxdy, unsigned int src_right,
						  unsigned int src_bottom, int src_span, int dest_span, int ia, int filtermode
#ifndef LICE_FAVOR_SPEED
						  ,
						  LICE_COMBINEFUNC combFunc
#endif
				...
				while (n--)
				{
					...
					else if (cury == src_bottom - 1)
					{
						if (curx < src_right - 1)
						{
							...
							__LICE_LinearFilterI(&r, &g, &b, &a, pin,
												 pin + sizeof(LICE_pixel) / sizeof(LICE_pixel_chan), thisx & 65535);
...
#define DOPIX(pout, r, g, b, a, ia) COMBFUNC::doPix(pout, r, g, b, a, ia);
  ...
  public:
	static void deltaBlitAlpha(LICE_pixel_chan* dest, const LICE_pixel_chan* src, int w, int h, int isrcx, int isrcy,
							   int idsdx, int idtdx, int idsdy, int idtdy, int idsdxdy, int idtdxdy,
							   unsigned int src_right, unsigned int src_bottom, int src_span, int dest_span, int ia,
							   int idadx, int idady, int idadxdy, int filtermode
#ifndef LICE_FAVOR_SPEED
							   ,
							   LICE_COMBINEFUNC combFunc
#endif
			...
			while (h--)
			{
				...
				while (n--)
				{
					...
					else if (cury == src_bottom - 1)
					{
						...
						{
							const LICE_pixel_chan* pin = src + cury * src_span + curx * sizeof(LICE_pixel);
							DOPIX(pout, pin[LICE_PIXEL_R], pin[LICE_PIXEL_G], pin[LICE_PIXEL_B], pin[LICE_PIXEL_A],
								  thisa >> 8);
						}
					}
...
void LICE_GradRect(LICE_IBitmap* dest, int dstx, int dsty, int dstw, int dsth, float ir, float ig, float ib, float ia,
				   float drdx, float dgdx, float dbdx, float dadx, float drdy, float dgdy, float dbdy, float dady,
				   int mode)
{
	if (!dest)
		return;
	...
	if (!pdest || !dest_span || dstw < 1 || dsth < 1 || dstx >= destbm_w || dsty >= destbm_h)
		return;
...
#define TOFIX(a) ((int)((a)*65536.0))
	...
	int iir = TOFIX(ir), iig = TOFIX(ig), iib = TOFIX(ib), iia = TOFIX(ia), idrdx = TOFIX(drdx), idgdx = TOFIX(dgdx),
		idbdx = TOFIX(dbdx), idadx = TOFIX(dadx), idrdy = TOFIX(drdy), idgdy = TOFIX(dgdy), idbdy = TOFIX(dbdy),
		idady = TOFIX(dady);
...
#define __LICE__ACTION(comb)                                                                                           \
	_LICE_Template_Blit1<comb>::gradBlit(pdest, dstw, dsth, iir, iig, iib, iia, idrdx, idgdx, idbdx, idadx, idrdy,     \
										 idgdy, idbdy, idady, dest_span)
#endif
...
#ifdef LICE_FAVOR_SIZE_EXTREME
	if (blitfunc)
		_LICE_Template_Blit1::gradBlit(pdest, dstw, dsth, iir, iig, iib, iia, idrdx, idgdx, idbdx, idadx, idrdy, idgdy,
									   idbdy, idady, dest_span, blitfunc);
#endif
...
static void LICE_BlitInt(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, const RECT* srcrect, float alpha,
						 int mode, bool allowSc)
{
	if (!dest || !src || !alpha)
		return;
	...
	if (sr.left < 0 || sr.top < 0)
		return; // overflow check
	if (sr.right <= sr.left || sr.bottom <= sr.top || dstx >= destbm_w || dsty >= destbm_h)
		return;

	if (sr.right > sr.left + (destbm_w - dstx))
		sr.right = sr.left + (destbm_w - dstx);
	if (sr.bottom > sr.top + (destbm_h - dsty))
		sr.bottom = sr.top + (destbm_h - dsty);
	...
	{
		LICE_Ext_Blit_acceldata data(src, dstx, dsty, sr.left, sr.top, sr.right - sr.left, sr.bottom - sr.top, alpha,
									 mode);
		if (dest->Extended(LICE_EXT_BLIT_ACCEL, &data))
			return;
	}
#endif
	...
	if ((mode & LICE_BLIT_MODE_MASK) >= LICE_BLIT_MODE_CHANCOPY &&
		(mode & LICE_BLIT_MODE_MASK) < LICE_BLIT_MODE_CHANCOPY + 0x10)
	{
		while (i-- > 0)
		{
	...
	else if ((mode & (LICE_BLIT_MODE_MASK | LICE_BLIT_USE_ALPHA)) == LICE_BLIT_MODE_COPY &&
			 (alpha == 1.0 || alpha == 0.5))
	{
		if (alpha == 0.5)
		{
...
#ifdef LICE_FAVOR_SIZE
		if (blitfunc)
			_LICE_Template_Blit2::blit(pdest, psrc, cpsize, i, src_span, dest_span, ia, blitfunc);
#endif
	}
}
...
void LICE_Blit(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, int srcx, int srcy, int srcw, int srch,
...
void LICE_Blur(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, int srcx, int srcy, int srcw,
			   int srch) // src and dest can overlap, however it may look fudgy if they do
{
	if (!dest || !src)
		return;
	...
	if (sr.top < 0)
		sr.top = 0;
	if (sr.right > src->getWidth())
		sr.right = src->getWidth();
	if (sr.bottom > src->getHeight())
		sr.bottom = src->getHeight();
	...
	if (sr.right <= sr.left + 1 || sr.bottom <= sr.top + 1)
		return;
	...
	if (src == dest)
	{
		if (w <= (int)(sizeof(turdbuf) / sizeof(turdbuf[0]) / 2))
			tmpbuf = turdbuf;
		else
			tmpbuf = (LICE_pixel*)malloc(w * 2 * sizeof(LICE_pixel));
	}
	...
	for (i = sr.top; i < sr.bottom; i++)
	{
		...
		if (i == sr.top || i == sr.bottom - 1)
		{
			...
			pdest[0] = LICE_PIXEL_HALF(lp = psrc[0]) + LICE_PIXEL_QUARTER(psrc[1]) + LICE_PIXEL_QUARTER(psrc2[0]);
			int x;
			for (x = 1; x < w - 1; x++)
			{
				LICE_pixel tp;
				pdest[x] = LICE_PIXEL_HALF(tp = psrc[x]) + LICE_PIXEL_QUARTER(psrc2[x]) +
						   LICE_PIXEL_EIGHTH(psrc[x + 1]) + LICE_PIXEL_EIGHTH(lp);
				lp = tp;
			}
			pdest[x] = LICE_PIXEL_HALF(psrc[x]) + LICE_PIXEL_QUARTER(lp) + LICE_PIXEL_QUARTER(psrc2[x]);
		}
		else
		{
			...
			LICE_pixel lp;
			pdest[0] = LICE_PIXEL_HALF(lp = psrc[0]) + LICE_PIXEL_QUARTER(psrc[1]) + LICE_PIXEL_EIGHTH(psrc2[0]) +
					   LICE_PIXEL_EIGHTH(psrc3[0]);
			int x;
			for (x = 1; x < w - 1; x++)
			{
				LICE_pixel tp;
				pdest[x] = LICE_PIXEL_HALF(tp = psrc[x]) + LICE_PIXEL_EIGHTH(psrc[x + 1]) + LICE_PIXEL_EIGHTH(lp) +
						   LICE_PIXEL_EIGHTH(psrc2[x]) + LICE_PIXEL_EIGHTH(psrc3[x]);
				lp = tp;
			}
			pdest[x] = LICE_PIXEL_HALF(psrc[x]) + LICE_PIXEL_QUARTER(lp) + LICE_PIXEL_EIGHTH(psrc2[x]) +
...
#ifndef LICE_NO_BLIT_SUPPORT
void LICE_ScaledBlit(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, int dstw, int dsth, float srcx,
					 float srcy, float srcw, float srch, float alpha, int mode)
{
	if (!dest || !src || !dstw || !dsth || !alpha)
		return;
	...
	if (dest->Extended(LICE_EXT_SUPPORTS_ID, (void*)LICE_EXT_SCALEDBLIT_ACCEL))
	{
		...
		if (dest->Extended(LICE_EXT_SCALEDBLIT_ACCEL, &data))
			return;
	}
#endif
	...
	if (dstw < 1 || dsth < 1 || dstx >= destbm_w || dsty >= destbm_h)
		return;
	...
	if ((mode & (LICE_BLIT_FILTER_MASK | LICE_BLIT_MODE_MASK | LICE_BLIT_USE_ALPHA)) == LICE_BLIT_MODE_COPY &&
		(ia == 128 || ia == 256))
	{
		if (ia == 128)
		{
			_LICE_Template_Blit0<_LICE_CombinePixelsHalfMixFAST>::scaleBlitFAST(
				pdest, psrc, dstw, dsth, icurx, icury, idx, idy, clip_r, clip_b, src_span, dest_span);
		}
		else
		{
			_LICE_Template_Blit0<_LICE_CombinePixelsClobberFAST>::scaleBlitFAST(
				pdest, psrc, dstw, dsth, icurx, icury, idx, idy, clip_r, clip_b, src_span, dest_span);
		}
	}
	else
...
#else
#define __LICE__ACTION(comb)                                                                                           \
	_LICE_Template_Blit2<comb>::scaleBlitFilterDown(pdest, psrc, dstw, dsth, icurx, icury, idx, idy, clip_r, clip_b,   \
													src_span, dest_span, ia, filter, filt_start, filtsz)
#endif
			__LICE_ACTION_SRCALPHA(mode, ia, false);
#undef __LICE__ACTION
...
#ifdef LICE_FAVOR_SIZE
			if (blitfunc)
				_LICE_Template_Blit2::scaleBlitFilterDown(pdest, psrc, dstw, dsth, icurx, icury, idx, idy, clip_r,
														  clip_b, src_span, dest_span, ia, filter, filt_start, filtsz,
														  blitfunc);
#endif
		}
		else
...
#else
#define __LICE__ACTION(comb)                                                                                           \
	_LICE_Template_Blit2<comb>::scaleBlit(pdest, psrc, dstw, dsth, icurx, icury, idx, idy, clip_r, clip_b, src_span,   \
										  dest_span, ia, mode& LICE_BLIT_FILTER_MASK)
#endif
			__LICE_ACTION_SRCALPHA(mode, ia, false);
#undef __LICE__ACTION
#ifdef LICE_FAVOR_SIZE
			if (blitfunc)
				_LICE_Template_Blit2::scaleBlit(pdest, psrc, dstw, dsth, icurx, icury, idx, idy, clip_r, clip_b,
												src_span, dest_span, ia, mode & LICE_BLIT_FILTER_MASK, blitfunc);
#endif
		}
	}
}

void LICE_DeltaBlit(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, int dstw, int dsth, float srcx,
					float srcy, float srcw, float srch, double dsdx, double dtdx, double dsdy, double dtdy,
					double dsdxdy, double dtdxdy, bool cliptosourcerect, float alpha, int mode)
{
	...
	if (cliptosourcerect)
	{
		...
		if (srcy > src_top)
			src_top = srcy;
		if (srcx + srcw < src_right)
			src_right = srcx + srcw;
		if (srcy + srch < src_bottom)
			src_bottom = srcy + srch;
	}
...
#else
#define __LICE__ACTION(comb)                                                                                           \
	_LICE_Template_Blit3<comb>::deltaBlit(pdest, psrc, dstw, dsth, isrcx, isrcy, idsdx, idtdx, idsdy, idtdy, idsdxdy,  \
										  idtdxdy, sr, sb, src_span, dest_span, ia, mode& LICE_BLIT_FILTER_MASK)
#endif
	__LICE_ACTION_SRCALPHA(mode, ia, false);
#undef __LICE__ACTION
...
#ifndef LICE_FAVOR_SPEED
	if (blitfunc)
		_LICE_Template_Blit3::deltaBlit(pdest, psrc, dstw, dsth, isrcx, isrcy, idsdx, idtdx, idsdy, idtdy, idsdxdy,
										idtdxdy, sr, sb, src_span, dest_span, ia, mode & LICE_BLIT_FILTER_MASK,
										blitfunc);
#endif
}

void LICE_DeltaBlitAlpha(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, int dstw, int dsth, float srcx,
						 float srcy, float srcw, float srch, double dsdx, double dtdx, double dsdy, double dtdy,
						 double dsdxdy, double dtdxdy, bool cliptosourcerect, float alpha, int mode, double dadx,
						 double dady, double dadxdy)
{
	if (!dest || !src || !dstw || !dsth)
		return;
	...
	{
		LICE_DeltaBlit(dest, src, dstx, dsty, dstw, dsth, srcx, srcy, srcw, srch, dsdx, dtdx, dsdy, dtdy, dsdxdy,
					   dtdxdy, cliptosourcerect, alpha, mode);
		return;
	}
...
#else
#define __LICE__ACTION(comb)                                                                                           \
	_LICE_Template_Blit4<comb>::deltaBlitAlpha(pdest, psrc, dstw, dsth, isrcx, isrcy, idsdx, idtdx, idsdy, idtdy,      \
											   idsdxdy, idtdxdy, sr, sb, src_span, dest_span, ia, idadx, idady,        \
											   idadxdy, mode& LICE_BLIT_FILTER_MASK)
#endif
	__LICE_ACTION_NOSRCALPHA(mode, 256, true);
#undef __LICE__ACTION
...
#ifndef LICE_FAVOR_SPEED
	if (blitfunc)
		_LICE_Template_Blit4::deltaBlitAlpha(pdest, psrc, dstw, dsth, isrcx, isrcy, idsdx, idtdx, idsdy, idtdy, idsdxdy,
											 idtdxdy, sr, sb, src_span, dest_span, ia, idadx, idady, idadxdy,
											 mode & LICE_BLIT_FILTER_MASK, blitfunc);
#endif
}

void LICE_RotatedBlit(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, int dstw, int dsth, float srcx,
					  float srcy, float srcw, float srch, float angle, bool cliptosourcerect, float alpha, int mode,
...
#else
#define __LICE__ACTION(comb)                                                                                           \
	_LICE_Template_Blit3<comb>::deltaBlit(pdest, psrc, dstw, dsth, isrcx, isrcy, idsdx, idtdx, idsdy, idtdy, 0, 0, sr, \
										  sb, src_span, dest_span, ia, mode& LICE_BLIT_FILTER_MASK)
#endif
	__LICE_ACTION_SRCALPHA(mode, ia, false);
#undef __LICE__ACTION
...
#ifndef LICE_FAVOR_SPEED
	if (blitfunc)
		_LICE_Template_Blit3::deltaBlit(pdest, psrc, dstw, dsth, isrcx, isrcy, idsdx, idtdx, idsdy, idtdy, 0, 0, sr, sb,
										src_span, dest_span, ia, mode & LICE_BLIT_FILTER_MASK, blitfunc);
#endif
}
...
void LICE_Clear(LICE_IBitmap* dest, LICE_pixel color)
{
	...
	{
		if (dest->Extended(LICE_EXT_CLEAR_ACCEL, &color))
			return;
	}
#endif
...
void LICE_MultiplyAddRect(LICE_IBitmap* dest, int x, int y, int w, int h, float rsc, float gsc, float bsc, float asc,
						  float radd, float gadd, float badd, float aadd)
{
	...
	LICE_pixel* p = dest->getBits();
	const int sp = dest->getRowSpan();
	if (!p || !sp || w < 1 || h < 1 || x >= destbm_w || y >= destbm_h)
		return;
	...
	while (h-- > 0)
	{
		...
		while (n--)
		{
			...
			_LICE_MakePixelClamp(ptr, (ptr[LICE_PIXEL_R] * ir + ir2) >> 8, (ptr[LICE_PIXEL_G] * ig + ig2) >> 8,
								 (ptr[LICE_PIXEL_B] * ib + ib2) >> 8, (ptr[LICE_PIXEL_A] * ia + ia2) >> 8);
		}
		p += sp - w;
	}
}

void LICE_ProcessRect(LICE_IBitmap* dest, int x, int y, int w, int h, void (*procFunc)(LICE_pixel* p, void* parm),
					  void* parm)
{
	if (!dest || !procFunc)
		return;
...
void LICE_FillRect(LICE_IBitmap* dest, int x, int y, int w, int h, LICE_pixel color, float alpha, int mode)
{
	...
	if (dest->Extended(LICE_EXT_SUPPORTS_ID, (void*)LICE_EXT_FILLRECT_ACCEL))
	{
		...
		if (dest->Extended(LICE_EXT_FILLRECT_ACCEL, &data))
			return;
	}
#endif

	if (mode & LICE_BLIT_USE_ALPHA)
		alpha *= LICE_GETA(color) / 255.0f;
	...
	if (!alpha || !p || !sp || w < 1 || h < 1 || x >= destbm_w || y >= destbm_h)
		return;
	...
	if ((mode & LICE_BLIT_MODE_MASK) == LICE_BLIT_MODE_COPY)
	{
		...
		if (ia == 128)
		{
			...
			_LICE_Template_Blit0<_LICE_CombinePixelsHalfMix2FAST>::solidBlitFAST(p, w, h, (color >> 1) & 0x7f7f7f7f,
																				 sp);
			return;
		}
		if (ia == 64)
		{
			_LICE_Template_Blit0<_LICE_CombinePixelsQuarterMix2FAST>::solidBlitFAST(p, w, h, (color >> 2) & 0x3f3f3f3f,
																					sp);
			return;
		}
		if (ia == 192)
		{
			_LICE_Template_Blit0<_LICE_CombinePixelsThreeQuarterMix2FAST>::solidBlitFAST(
				p, w, h, ((color >> 1) & 0x7f7f7f7f) + ((color >> 2) & 0x3f3f3f3f), sp);
			return;
		}
	}
...
#else
#define __LICE__ACTION(comb)                                                                                           \
	_LICE_Template_Blit1<comb>::solidBlit((LICE_pixel_chan*)p, w, h, LICE_GETR(color), LICE_GETG(color),               \
										  LICE_GETB(color), ia, sp * sizeof(LICE_pixel))
#endif
...
#ifdef LICE_FAVOR_SIZE_EXTREME
	if (blitfunc)
		_LICE_Template_Blit1::solidBlit((LICE_pixel_chan*)p, w, h, LICE_GETR(color), LICE_GETG(color), LICE_GETB(color),
										ia, sp * sizeof(LICE_pixel), blitfunc);
#endif
}
...
LICE_pixel LICE_GetPixel(LICE_IBitmap* bm, int x, int y)
{
	...
	if (bm->Extended(LICE_EXT_SUPPORTS_ID, (void*)LICE_EXT_GETPIXEL_ACCEL))
	{
		...
		if (bm->Extended(LICE_EXT_GETPIXEL_ACCEL, &data))
			return data.px;
	}
#endif
	...
	if (!(px = bm->getBits()) || x < 0 || y < 0 || x >= w || y >= h)
		return 0;
	if (bm->isFlipped())
		return px[(h - 1 - y) * bm->getRowSpan() + x];
...
void LICE_PutPixel(LICE_IBitmap* bm, int x, int y, LICE_pixel color, float alpha, int mode)
{
	...
	if (bm->Extended(LICE_EXT_SUPPORTS_ID, (void*)LICE_EXT_PUTPIXEL_ACCEL))
	{
		...
		if (bm->Extended(LICE_EXT_PUTPIXEL_ACCEL, &data))
			return;
	}
#endif
...
{
  public:
	static void blit(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, int dstw, int dsth, const T* srcpoints,
					 int div_w,
					 int div_h, // srcpoints coords should be div_w*div_h*2 long, and be in source image coordinates
					 float alpha, int mode)
	{
		if (!dest || !src || dstw < 1 || dsth < 1 || div_w < 2 || div_h < 2)
			return;
			...
			if (nypos != cypos)
			{
				...
				for (x = 0; x < div_w - 1; x++)
				{
					...
					if (nxpos != cxpos)
					{
						...
						LICE_DeltaBlit(dest, src, cxpos, cypos, nxpos - cxpos, nypos - cypos, (float)sx, (float)sy,
									   (float)sw, (float)sh, dsdx, dtdx, dsdy, dtdy, dsdxdy, dtdxdy, false, alpha,
									   mode);
					}
			...
			if (nypos != cypos)
			{
				...
				for (x = 0; x < div_w - 1; x++)
				{
					...
					if (nxpos != cxpos)
					{
						...
						LICE_DeltaBlitAlpha(dest, src, cxpos, cypos, nxpos - cxpos, nypos - cypos, (float)sx, (float)sy,
											(float)sw, (float)sh, dsdx, dtdx, dsdy, dtdy, dsdxdy, dtdxdy, false, sa,
											mode, dadx, dady, dadxdy);
					}
...
void LICE_TransformBlit(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, int dstw, int dsth,
						const float* srcpoints, int div_w,
						int div_h, // srcpoints coords should be div_w*div_h*2 long, and be in source image coordinates
						float alpha, int mode)
{
	LICE_TransformBlit_class<float>::blit(dest, src, dstx, dsty, dstw, dsth, srcpoints, div_w, div_h, alpha, mode);
}
void LICE_TransformBlit2(LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, int dstw, int dsth,
						 const double* srcpoints, int div_w,
						 int div_h, // srcpoints coords should be div_w*div_h*2 long, and be in source image coordinates
						 float alpha, int mode)
{
...
void LICE_TransformBlit2Alpha(
	LICE_IBitmap* dest, LICE_IBitmap* src, int dstx, int dsty, int dstw, int dsth, const double* srcpoints, int div_w,
	int div_h, // srcpoints coords should be div_w*div_h*3 long, and be in source image coordinates+alpha
	int mode)
{
...
void LICE_SetAlphaFromColorMask(LICE_IBitmap* dest, LICE_pixel color)
{
	...
	while (h-- > 0)
	{
		...
		while (n--)
		{
			if ((*p & LICE_RGBA(255, 255, 255, 0)) == color)
				*p &= LICE_RGBA(255, 255, 255, 0);
			else
				*p |= LICE_RGBA(0, 0, 0, 255);
			p++;
		}
		p += sp - w;
	}
...
{
  public:
	static void DrawGlyph(const LICE_pixel_chan* srcalpha, LICE_pixel* destpx, int src_w, int src_h, LICE_pixel color,
						  int span, int src_span, int aa)
	{
	...
	static void DrawGlyphScale(const LICE_pixel_chan* srcalpha, LICE_pixel* destpx, int src_w, int src_h,
							   LICE_pixel color, int span, int src_span, int aa, int scale)
	{
		const int r = LICE_GETR(color), g = LICE_GETG(color), b = LICE_GETB(color), a = LICE_GETA(color);
	...
	static void DrawGlyphMono(const unsigned char* srcalpha, LICE_pixel* destpx, int src_w, int src_h, LICE_pixel color,
							  int span, int src_span, int aa)
	{
		const int r = LICE_GETR(color), g = LICE_GETG(color), b = LICE_GETB(color), a = LICE_GETA(color);
	...
	static void DrawGlyphMonoScale(const unsigned char* srcalpha, LICE_pixel* destpx, int src_w, int src_h,
								   LICE_pixel color, int span, int src_span, int aa, int scale)
	{
		const int r = LICE_GETR(color), g = LICE_GETG(color), b = LICE_GETB(color), a = LICE_GETA(color);
...
void LICE_DrawGlyphEx(LICE_IBitmap* dest, int x, int y, LICE_pixel color, const LICE_pixel_chan* alphas, int glyph_w,
					  int glyph_span, int glyph_h, float alpha, int mode)
{
	if (!dest)
		return;
	...
	if (dest->Extended(LICE_EXT_SUPPORTS_ID, (void*)LICE_EXT_DRAWGLYPH_ACCEL) && glyph_w == glyph_span)
	{
		...
		if (dest->Extended(LICE_EXT_DRAWGLYPH_ACCEL, &data))
			return;
	}
#endif
	...
	if (src_w < 0 || src_h < 0 || x >= destbm_w || y >= destbm_h)
		return;
...
void LICE_DrawGlyph(LICE_IBitmap* dest, int x, int y, LICE_pixel color, const LICE_pixel_chan* alphas, int glyph_w,
					int glyph_h, float alpha, int mode)
{
	LICE_DrawGlyphEx(dest, x, y, color, alphas, glyph_w, glyph_w, glyph_h, alpha, mode);
}

void LICE_DrawMonoGlyph(LICE_IBitmap* dest, int x, int y, LICE_pixel color, const unsigned char* alphabits, int glyph_w,
						int glyph_span, int glyph_h, float alpha, int mode)
{
	if (!dest)
		return;
	...
	if (glyph_span < 0)
		alphabits += -glyph_span * (glyph_h - 1);
...
void LICE_HalveBlitAA(LICE_IBitmap* dest, LICE_IBitmap* src)
{
	...
	while (h--)
	{
		...
		while (x--) // unroll 2px at a time, about 5% faster on core2 and ICC
		{
			...
			dp[0] = r1 + ((sp2[0] >> 2) & 0x3f3f3f3f) + ((sp2[1] >> 2) & 0x3f3f3f3f);
			dp[1] = r2 + ((sp2[2] >> 2) & 0x3f3f3f3f) + ((sp2[3] >> 2) & 0x3f3f3f3f);
			sp += 4;
			sp2 += 4;
			dp += 2;
		}
		if (w & 1)
		{
			*dp = ((sp[0] >> 2) & 0x3f3f3f3f) + ((sp[1] >> 2) & 0x3f3f3f3f) + ((sp2[0] >> 2) & 0x3f3f3f3f) +
				  ((sp2[1] >> 2) & 0x3f3f3f3f);
		}
		srcptr += src_span * 2;
		destptr += dest_span;
...
unsigned short _LICE_RGB2HSV_invtab[256] = {
	// 65536/idx - 1
	0,		0xffff, 0x7fff, 0x5554, 0x3fff, 0x3332, 0x2aa9, 0x2491, 0x1fff, 0x1c70, 0x1998, 0x1744, 0x1554, 0x13b0,
	0x1248, 0x1110, 0x0fff, 0x0f0e, 0x0e37, 0x0d78, 0x0ccb, 0x0c2f, 0x0ba1, 0x0b20, 0x0aa9, 0x0a3c, 0x09d7, 0x097a,
	0x0923, 0x08d2, 0x0887, 0x0841, 0x07ff, 0x07c0, 0x0786, 0x074f, 0x071b, 0x06ea, 0x06bb, 0x068f, 0x0665, 0x063d,
	0x0617, 0x05f3, 0x05d0, 0x05af, 0x058f, 0x0571, 0x0554, 0x0538, 0x051d, 0x0504, 0x04eb, 0x04d3, 0x04bc, 0x04a6,
	0x0491, 0x047c, 0x0468, 0x0455, 0x0443, 0x0431, 0x0420, 0x040f, 0x03ff, 0x03ef, 0x03df, 0x03d1, 0x03c2, 0x03b4,
	0x03a7, 0x039a, 0x038d, 0x0380, 0x0374, 0x0368, 0x035d, 0x0352, 0x0347, 0x033c, 0x0332, 0x0328, 0x031e, 0x0314,
	0x030b, 0x0302, 0x02f9, 0x02f0, 0x02e7, 0x02df, 0x02d7, 0x02cf, 0x02c7, 0x02bf, 0x02b8, 0x02b0, 0x02a9, 0x02a2,
	0x029b, 0x0294, 0x028e, 0x0287, 0x0281, 0x027b, 0x0275, 0x026f, 0x0269, 0x0263, 0x025d, 0x0258, 0x0252, 0x024d,
	0x0248, 0x0242, 0x023d, 0x0238, 0x0233, 0x022f, 0x022a, 0x0225, 0x0221, 0x021c, 0x0218, 0x0213, 0x020f, 0x020b,
	0x0207, 0x0203, 0x01ff, 0x01fb, 0x01f7, 0x01f3, 0x01ef, 0x01eb, 0x01e8, 0x01e4, 0x01e0, 0x01dd, 0x01d9, 0x01d6,
	0x01d3, 0x01cf, 0x01cc, 0x01c9, 0x01c6, 0x01c2, 0x01bf, 0x01bc, 0x01b9, 0x01b6, 0x01b3, 0x01b1, 0x01ae, 0x01ab,
	0x01a8, 0x01a5, 0x01a3, 0x01a0, 0x019d, 0x019b, 0x0198, 0x0196, 0x0193, 0x0191, 0x018e, 0x018c, 0x0189, 0x0187,
	0x0185, 0x0182, 0x0180, 0x017e, 0x017c, 0x0179, 0x0177, 0x0175, 0x0173, 0x0171, 0x016f, 0x016d, 0x016b, 0x0169,
	0x0167, 0x0165, 0x0163, 0x0161, 0x015f, 0x015d, 0x015b, 0x0159, 0x0157, 0x0156, 0x0154, 0x0152, 0x0150, 0x014f,
	0x014d, 0x014b, 0x0149, 0x0148, 0x0146, 0x0145, 0x0143, 0x0141, 0x0140, 0x013e, 0x013d, 0x013b, 0x013a, 0x0138,
	0x0137, 0x0135, 0x0134, 0x0132, 0x0131, 0x012f, 0x012e, 0x012d, 0x012b, 0x012a, 0x0128, 0x0127, 0x0126, 0x0124,
	0x0123, 0x0122, 0x0120, 0x011f, 0x011e, 0x011d, 0x011b, 0x011a, 0x0119, 0x0118, 0x0117, 0x0115, 0x0114, 0x0113,
	0x0112, 0x0111, 0x0110, 0x010e, 0x010d, 0x010c, 0x010b, 0x010a, 0x0109, 0x0108, 0x0107, 0x0106, 0x0105, 0x0104,
	0x0103, 0x0102, 0x0101, 0x0100,
};

#endif //__LICE_CPP_IMPLEMENTED__

/* CUSTOM: end pragma warning */
#pragma warning(pop)
